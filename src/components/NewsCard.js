...
const NewsCard = ({ article }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 dark:border dark:border-gray-700/50 flex flex-col h-full group backdrop-blur-sm">
      <div className="relative overflow-hidden aspect-video">
        {article.urlToImage ? (
          <>
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-gray-900/80 via-gray-800/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
          </>
        ) : (
        ...
        {/* Date badge */}
        <div className="absolute bottom-3 right-3 bg-white/90 dark:bg-gray-800/90 text-slate-700 dark:text-gray-300 text-xs font-medium py-1 px-2 rounded-md backdrop-blur-sm">
          {formatDate(article.publishedAt)}
        </div>
      </div>
      
      <div className="p-5 flex flex-col flex-grow">
        <h2 className="text-xl font-bold mb-3 line-clamp-2 text-slate-800 dark:text-gray-100 group-hover:text-slate-600 dark:group-hover:text-gray-200 transition-colors duration-300">
          {article.title}
        </h2>
        ...
        <div className="flex items-center text-xs text-gray-500 dark:text-gray-400 mb-4">
          {article.author && (
            <div className="flex items-center mr-3 bg-gray-100 dark:bg-gray-700/30 px-2 py-1 rounded-full">
